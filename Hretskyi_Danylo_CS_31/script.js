...
function openCell(row, col) {
    if (game.status !== GAME_STATE.PLAYING) return;      // 1) якщо гра вже не активна — нічого не робимо
        ...
        for (let deltaRow = -1; deltaRow <= 1; deltaRow++) {
          for (let deltaCol = -1; deltaCol <= 1; deltaCol++) {
            ...
            if (inBounds(rr, cc, game.rows, game.cols)) { // перевірка, чи клітинка в межах поля
              ...
              if (neighbourCell.state === CELL_STATE.COVERED && !neighbourCell.mine) stack.push([rr, cc]); // якщо клітинка закрита і не міна, то додаємо в стек
            }
          }
        }
      }
...
function toggleFlag(row, col) {
    if (game.status !== GAME_STATE.PLAYING) return;                    // гра має бути активна
    if (!inBounds(row, col, game.rows, game.cols)) return;       // перевірка меж
  
    const cell = game.board[row][col];
  
    if (cell.state === CELL_STATE.UNCOVERED) return;                        // відкриті не флагуємо
